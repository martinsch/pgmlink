...
:  gm_(gm) 
{
   ...
   for(size_t node = 0; node < gm_.numberOfVariables(); ++node) {
      ...
      constraint_name << "indicator variables constraint = 1, node = " << node;
      c_.add(IloRange(env_, 1, 1, constraint_name.str().c_str()));
      for(size_t i = 0; i < gm_.numberOfLabels(node); ++i) {
   ...
   for(size_t f = 0; f < gm_.numberOfFactors(); ++f) {
      if(gm_[f].numberOfVariables() > 1) {
         ...
         for(size_t n = 0; n < gm_[f].numberOfVariables(); ++n) {
            ...
            for(size_t i = 0; i < gm_.numberOfLabels(node); ++i) {
               std::stringstream constraint_name;
               constraint_name << "indicator variables (factors) constraint = 0, f = " << f 
                    << ", n = " << n << ", i = " << i;
               c_.add(IloRange(env_, 0, 0, constraint_name.str().c_str()));
               c_[constraintCounter].setLinearCoef(x_[idNodesBegin_[node]+i], -1);
   ...
   try {
	...
	cplex_.setParam(IloCplex::SiftDisplay, 0);
      ...
      if(!cplex_.solve()) {
         ...
         //if (cplex_.getStatus() == IloCplex::IloAlgorithm::Infeasible ||
         //   cplex_.getStatus() == IloCplex::IloAlgorithm::InfeasibleOrUnbound) {
