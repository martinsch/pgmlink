...
vector<vector<Event> > BotTracking::operator()(TraxelStore& ts) {
	...
	SingleTimestepTraxelMrf mrf(detection, misdetection, appearance,
			disappearance, move, division, opportunity_cost_);
...
vector<map<unsigned int, bool> > KanadeTracking::detections() {
	...
	if (last_detections_) {
		...
		throw std::runtime_error(
				"KanadeTracking::detections(): previous tracking result required");
	}
}
...
vector<Event> Track::operator()(const Traxels& prev, const Traxels& curr) {
	...
	//// Formulate ilp
	////
	pair<shared_ptr<AdaptiveEnergiesIlp>, vector<Event> > ret =
			formulation_.formulate_ilp(prev, curr);
	...
	if (ilp->nVars > 0) {
		...
		int err;
		err = solver.solve(ilp->nVars, ilp->nConstr, ilp->nNonZero, ilp->costs,
				ilp->rhs, ilp->matbeg, ilp->matcnt, ilp->matind, ilp->matval,
				string("Adaptive Energies Approach"), finalCost, finalVars,
				solverOutput, outputString);
		if (err != 0) {
	...
	////
	shared_ptr<vector<Event> > events = events_from(prev, curr, finalVars,
			ilp->nVars, ilp_events);

	return *events;
}

shared_ptr<vector<Event> > Track::events_from(const Traxels& prev,
		const Traxels& curr, shared_array<double> ilp_finalVars, int ilp_nVars,
	...
	assert(
			static_cast<vector<Event>::size_type>(ilp_nVars) == ilp_events.size());
	for (int i = 0; i < ilp_nVars; ++i) {
		...
		if (ilp_finalVars[i] == 1) {
			if (ilp_events[i].type == Event::Move) {
				...
				shared_ptr<const BinaryEnergy> energy_fct =
						formulation_.Move_energy();
				e.energy = energy_fct->operator()(
						prev.find(e.traxel_ids[0])->second,
						curr.find(e.traxel_ids[1])->second, prev, curr);
				events->push_back(e);
				...
				shared_ptr<const TertiaryEnergy> energy_fct =
						formulation_.Division_energy();
				e.energy = energy_fct->operator()(
						prev.find(e.traxel_ids[0])->second,
						curr.find(e.traxel_ids[1])->second,
						curr.find(e.traxel_ids[2])->second, prev, curr);
				events->push_back(e);
			} else {
	...
	for (set<unsigned int>::const_iterator it = prev_ids.begin();
			it != prev_ids.end(); ++it) {
		e.type = Event::Disappearance;
		e.traxel_ids.clear();
		e.traxel_ids.push_back(*it);
		shared_ptr<const UnaryEnergy> energy_fct =
				formulation_.Disappearance_energy();
		e.energy = energy_fct->operator()(prev.find(*it)->second, prev, curr);
		events->push_back(e);
	}
	// construct appearances
	for (set<unsigned int>::const_iterator it = curr_ids.begin();
			it != curr_ids.end(); ++it) {
		e.type = Event::Appearance;
		e.traxel_ids.clear();
		e.traxel_ids.push_back(*it);
		shared_ptr<const UnaryEnergy> energy_fct =
				formulation_.Appearance_energy();
...
////
FixedCostTracking::FixedCostTracking(double division, double move,
		double disappearance, double appearance, double distance_threshold) {
	...
	formulation_ = AdaptiveEnergiesFormulation(div, mov, dis, app,
			distance_threshold);
}

vector<Event> FixedCostTracking::operator()(const Traxels& prev,
		const Traxels& curr) {
...
////
ShortestDistanceTracking::ShortestDistanceTracking(double division,
		double disappearance, double appearance, double distance_threshold,
	...
	shared_ptr<const ConstantEnergy> dis(new ConstantEnergy(disappearance));
	formulation_ = AdaptiveEnergiesFormulation(div, mov, dis, app,
			distance_threshold, "com", max_nearest_neighbors);
}

vector<Event> ShortestDistanceTracking::operator()(const Traxels& prev,
		const Traxels& curr) {
	Track track(formulation_);
	return track(prev, curr);
}
...
CellnessTracking::CellnessTracking(const std::string& random_forest_file,
		double w_div1, double w_div2, double w_move, double w_app,
		double w_disapp, double distance_threshold) :
	...
	shared_ptr<const CellnessDivision> div(
			new CellnessDivision(w_div1, w_div2));
	...
	shared_ptr<const CellnessDisappearance> dis(
			new CellnessDisappearance(w_disapp));
	formulation_ = AdaptiveEnergiesFormulation(div, mov, dis, app,
			distance_threshold);
}
