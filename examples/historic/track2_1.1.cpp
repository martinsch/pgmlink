...
const int SUBVERSION = 1; // Subversion of program
const string DATE_STRING = "2010/04/29"; // Date of current version
...
int MatchingPars::readOptions(int nOpts, char* opts[]){
  ...
  nOpts/=2;
  const string allowedKeys("kmsdaf");
...
// then all features for cell no. 2, ...).
  int readInputs(int& nCells, int& nFeats, boost::shared_array<double>& feats);
...
FileReader::FileReader( const string& fileName ) : fileName_(fileName),
  nCells_(0) {
}
...
public:
// Constructor
  FileReaderEmbl( const std::string& fileName );
...
FileReaderEmbl::FileReaderEmbl( const string& fileName ) : FileReader(fileName){
}
...
int FileReaderEmbl::readInputsSpecific( int& nCells, int& nFeats, 
  ...
  for( int iC=0; iC<nCells; iC++ ){
    for( int iF=0; iF<nFeats; iF++ ){
      feats[idx++] = featsBuf[iF*nCells+iC];
    }
  } 
  featsBuf.reset();
  if( err<0 ){
...
// Constructor with initialization of admissible named features
  FileReaderHci( const std::string& fileName );
...
private:
  ...
  struct NamedFeature {
    ...
    int nSubFeats_;    // how many subfeatures this feature comprises, e.g.
                       // 3 for a position
    double weight_; // inverse weight to be used for this feature
  };
...
FileReaderHci::FileReaderHci( const string& fileName ) : FileReader(fileName),
  namedFlags_(), flag_(0), labelValidity_() {
  NamedFeature position = {"position", 3, 1. };
  namedFlags_[0x100] = position;
  NamedFeature com = {"com", 3, 1.};
  namedFlags_[0x200] = com;
  NamedFeature intensity = {"intensity", 1, 1.};
    ...
    if( newLabelValidity[i] ){
      ...
      for( vector<unsigned long>::const_iterator it=allFlags.begin(); 
        ...
        for( int j = 0; j < currFeat.nSubFeats_; ++j ){
          feats[arrayIdx++] = doubleBuffer[j] * currFeat.weight_;
        }
      }
    }
  }
