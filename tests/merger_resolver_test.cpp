...
BOOST_AUTO_TEST_CASE( MergerResolver_extract_merger_coordinates ) {
  LOG(logDEBUG) << "MergerResolver_extract_merger_coordinates";
  // create the test image
  unsigned int image_array[9] = {
    1, 1, 0,
    0, 0, 3,
    0, 2, 3
  };
  vigra::MultiArray<2, unsigned int> image(vigra::Shape2(3, 3));
  for(size_t n = 0; n < 9; n++) {
    image[n] = image_array[n];
  }

  // create the vector with the offset
  vigra::TinyVector<unsigned int, 2> offset(1, 0);

  // create the MultiArrayView
  vigra::MultiArrayView<2, unsigned int> sub_image = image.subarray(
    vigra::Shape2(1, 0),
    vigra::Shape2(3, 3)
  );
  ...
  event1.type = Event::Merger;
  event2.type = Event::Move;
  event3.type = Event::Merger;

  event1.traxel_ids.push_back(1); event1.traxel_ids.push_back(2);
  event2.traxel_ids.push_back(2); event2.traxel_ids.push_back(2);
  event3.traxel_ids.push_back(3); event3.traxel_ids.push_back(2);
  std::vector<Event> event_vector;
  // create event vector with duplicate of event 1
  ...
  TimestepIdCoordinateMapPtr coordinate_map_ptr(new TimestepIdCoordinateMap);
  extract_merger_coordinates<2, unsigned int>(coordinate_map_ptr, sub_image, offset, event_vector, 1);

  TimestepIdCoordinateMap coordinate_map = *coordinate_map_ptr;
  
  LOG(logDEBUG) << "Entries in the coordinate map:";
  for(
    TimestepIdCoordinateMap::const_iterator it = coordinate_map.begin();
    it != coordinate_map.end();
    it++
  ) {
    std::pair<unsigned int, int> key = it->first;
    LOG(logDEBUG) << "Coordiates of traxel " << key.second
                  << " at timestep " << key.first;
    LOG(logDEBUG) << it->second;
  }

  BOOST_CHECK_EQUAL(coordinate_map.size(), 2);
  
  arma::mat m1 = coordinate_map[std::make_pair(1,1)];
  BOOST_CHECK_EQUAL(m1.n_rows, 2);
  BOOST_CHECK_EQUAL(m1.n_cols, 1);
  BOOST_CHECK_EQUAL(m1(0,0), 1);
  BOOST_CHECK_EQUAL(m1(1,0), 0);

  arma::mat m2 = coordinate_map[std::make_pair(1,3)];
  BOOST_CHECK_EQUAL(m2.n_rows, 2);
  BOOST_CHECK_EQUAL(m2.n_cols, 2);
  BOOST_CHECK_EQUAL(m2(0,0), 2); BOOST_CHECK_EQUAL(m2(0,1), 2);
  BOOST_CHECK_EQUAL(m2(1,0), 1); BOOST_CHECK_EQUAL(m2(1,1), 2);
}
