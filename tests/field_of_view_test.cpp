...
BOOST_AUTO_TEST_CASE( FieldOfView_test ) {
  ...
  pair<vector<double> > bb = fov.boundingbox();
  BOOST_CHECK_EQUAL_COLLECTIONS(bb.first.begin(), bb.first.end(), should_lb.begin(), should_lb.end());
  BOOST_CHECK_EQUAL_COLLECTIONS(bb.second.begin(), bb.second.end(), should_ub.begin(), should_ub.end());
  ...
  BOOST_CHECK_EQUAL(fov.spatial_margin( 3,2,1,1 ), 1);
